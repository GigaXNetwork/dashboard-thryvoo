...
const PresetCard = ({
  ...
  const formatExpiryDuration = (days, hours) => {
    const parts = [];
  ...
  // Calculate expiry date from creation date + duration
  const calculateExpiryDate = (createdAt, days, hours) => {
    ...
    if (hours) expiry.setHours(expiry.getHours() + parseInt(hours));
  ...
  return (
    <div className="relative bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 group overflow-hidden flex flex-col h-full">
      ...
      <div className="p-4 sm:p-5 pb-16 flex-1 flex flex-col">
        ...
        {/* Key Metrics */}
        <div className={`grid ${preset.maxDiscount ? 'grid-cols-3' : 'grid-cols-2'} gap-1.5 sm:gap-2 mb-3 sm:mb-4`}>
          {[
            { label: "Max Discount", value: preset.maxDiscount, icon: "📈" },
            { label: "Min Purchase", value: preset.minPurchase, icon: "💰" },
            { label: "Usage Limit", value: preset.usageLimit, icon: "🎯" },
        ...
        {/* Expiry Duration - Primary display */}
        {(preset.day || preset.hour) && (
          <div className="bg-gray-50 border border-gray-200 p-2 sm:p-3 rounded-lg mb-3">
            <div className="flex items-center gap-2 text-xs sm:text-sm font-medium text-gray-700 mb-2">
              <Clock className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
              <span className="truncate">Expiry Duration</span>
            </div>
            <div className="text-xs text-gray-600">
              <span className="break-words">
                {formatExpiryDuration(preset.day, preset.hour)} from creation date
              </span>
            </div>

            {/* Show calculated expiry date as additional info */}
            {preset.createdAt && (
              <div className="text-xs text-gray-500 mt-2 font-semibold">
                Expiry At : {calculateExpiryDate(preset.createdAt, preset.day, preset.hour)}
              </div>
            )}
          </div>
        )}

        {/* Fixed Expiry Date - Only show when no expiry duration */}
        {!(preset.day || preset.hour) && preset.expireAt && (
          <div className="bg-gray-50 border border-gray-200 p-2 sm:p-3 rounded-lg mb-3">
            <div className="flex items-center gap-2 text-xs sm:text-sm font-medium text-gray-700 mb-2">
              <Calendar className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
            ...
            <div className="text-xs text-gray-600 flex items-start gap-1">
              <Clock className="w-3 h-3 mt-0.5 flex-shrink-0" />
              <span className="break-words">
                {new Date(preset.expireAt).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'short',
                  day: 'numeric',
                  hour: '2-digit',
        ...
        {/* Validity Period - Keep existing for backward compatibility */}
        {preset.startAt && preset.expireAt && (
          <div className="bg-gray-50 border border-gray-200 p-2 sm:p-3 rounded-lg mb-3">
            <div className="flex items-center gap-2 text-xs sm:text-sm font-medium text-gray-700 mb-2">
              <Calendar className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
